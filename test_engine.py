...
# ------------------------------------------------- the leak proof itself

# The leak proof is the single check standing between this project and a
# model that has quietly seen its own answers. It has now failed at being a
# check twice: once by comparing columns that were never recomputed (so it
# could only ever say INCONCLUSIVE), and once by tampering upward with the
# day's HIGHEST scorer (so the value crossed no split threshold and a
# blatantly leaky model was waved through).
#
# So the proof gets its own pair of tests: one model that is honest, one that
# is handed the answer, and the check has to tell them apart. If these two
# ever agree, the proof has stopped proving anything again.

def _leak_frame(periods=16, players=40, seed=5):
    """Points on a realistic scale, unlike the fixture above.

    This matters: with points running 0-400, no plausible tamper value can
    cross a learned split threshold, and the test silently loses its teeth.
    """
    rng = np.random.default_rng(seed)
    rows = []
    for t in range(1, periods + 1):
        for p in range(players):
            side = "L" if p < players // 2 else "R"
            rows.append({
                "player_id": f"{p:03d}", "season": 2025, "period": t,
                "team": side, "opponent": "R" if side == "L" else "L",
                "is_home": 1 if side == "L" else 0,
                "position": ["A", "B", "C"][p % 3],
                "grabs": float(rng.integers(0, 10)),
                "runs": float(rng.integers(0, 10)),
                "points": float(max(0.0, rng.normal(9, 5))),
            })
    return pd.DataFrame(rows)


_LEAK_BASE = dict(usage=["grabs", "runs"], shares=["grabs"],
                  touch_columns=["grabs", "runs"], positions=["A", "B", "C"],
                  min_prior_games=2, quantiles=[0.10, 0.50, 0.90],
                  halflife=3.0, min_train_rows=50, min_played_rows=40)


def _leak_verdict(extra_features):
    from engine import grade as G
    spec = SportSpec(name="LeakToy", extra_features=extra_features,
                     **_LEAK_BASE)
    built = EF.build(_leak_frame(), spec, validate=False)
    return G.prove_no_leak(built, spec, 2025, 12)


def test_the_leak_proof_passes_an_honest_model():
    out = _leak_verdict([])
    assert "PASS" in out, f"an honest model was not cleared:\n{out}"


def test_the_leak_proof_fails_a_model_handed_the_answer():
    """`points` declared as its own feature. If this passes, nothing is safe."""
    out = _leak_verdict(["points"])
    assert "FAIL" in out, (
        f"a model fitted on the outcome it predicts was NOT caught:\n{out}")
