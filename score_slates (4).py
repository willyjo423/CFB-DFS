...
def find_board(actual: dict) -> Path | None:
    ...
    The threshold is high, and it is high because of a specific wrong answer.
    Baseball rosters barely move day to day, so a board from a DIFFERENT date
    still shares most of its names with any given contest. At a 50% bar, a
    contest played on 2026-09-16 matched a board generated on the 18th, and
    the report graded Tuesday's results against Thursday's projections
    without complaint.
    """
    ...
    share, size, path = best[0]

    # 90%, not 50%. Baseball rosters barely change from one day to the next,
    # so a DIFFERENT day's board shares most of its names with this contest -
    # on 2026-09-18 the wrong board scored 67% and was accepted, and the
    # report that followed compared one slate's projections against another
    # slate's results. Every number in it was wrong and none of them looked
    # wrong, which is the expensive kind. The right board contains nearly
    # every player the contest priced, because it IS that slate.
    if share < 0.90:
        print(f"NO BOARD MATCHES THIS CONTEST.\n"
              f"The closest is {path}, which holds {100 * share:.0f}% of the "
              f"contest's {len(names)} players - not enough.\n"
              f"A board from a different day shares most of its names with "
              f"this one, so anything under 90% is a different slate, and "
              f"grading against it would compare one slate's projections to "
              f"another slate's results.\n"
              f"The usual cause is that the board for the slate you played "
              f"has been replaced by a later publish.\n"
              f"Candidates:", file=sys.stderr)
        for s, n, p in best[:6]:
            print(f"  {100 * s:5.0f}%  {n:>4} players  {p}", file=sys.stderr)
        return None

    # Even at 90%+, a board three times the size of the contest is a bigger
    # slate that happens to contain this one - an all-day board against an
    # evening-only contest, say.
    if size > 1.8 * len(names):
        print(f"REFUSING {path}: it holds {size} players against the "
              f"contest's {len(names)}.\n"
              f"That is a larger slate that contains this one, not the slate "
              f"itself, and its ownership is spread over a different pool.",
              file=sys.stderr)
        return None

    print(f"matched to {path} ({100 * share:.0f}% of the contest's "
          f"{len(names)} players are on it, {size} on the board)")
    try:
        when = json.loads(path.read_text()).get("generated_at", "")
        if when:
            print(f"  that board was generated {when}. Check it is the day "
                  f"you played - the contest export carries no date.")
    except Exception:                                          # noqa: BLE001
        pass
    if len(best) > 1 and best[1][0] > share - 0.05:
        print(f"  note: {best[1][2]} is close behind at "
              f"{100 * best[1][0]:.0f}%")
    return path
