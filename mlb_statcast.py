...
# A week at a time, and then SMALLER if a week comes back at the cap.
#
# Savant truncates a CSV export at 25,000 rows and does not say so - no
# warning, no header, no partial-content status. It simply returns 25,000
# rows and stops. A full in-season week is over 30,000 pitches, so every
# week-sized request was silently losing its back end, and the losses were
# not random: they were whole games at the end of the range.
#
# The first version of this shipped that way and looked perfect. Twenty-eight
# of thirty-five weeks came back at exactly 25,000 pitches, the medians were
# sensible, every test passed, and roughly a fifth of the season was missing.
# Exactly 25,000 twenty-eight times is not a pitch count; it is a ceiling.
#
# So the cap is detected rather than avoided by guesswork: any range that
# returns it is split in half and re-fetched, recursively, until nothing
# comes back at the limit. A week that is genuinely small costs one request
# as before; a busy one costs three.
CHUNK_DAYS = 7

# Savant's export ceiling. Treated as "at or above" rather than "equal to" so
# that a change to 50,000 is still detected instead of silently passing.
EXPORT_CAP = 25_000

# How far the halving may go. One day of baseball is about 4,400 pitches, so
# a single day should never hit the cap - if it does, something has changed
# and the run says so rather than recursing forever.
MAX_SPLIT_DEPTH = 5
...
def _fetch_window(start: date, stop: date, depth: int = 0) -> pd.DataFrame:
    """Every pitch between two dates, splitting if the export was truncated.

    The recursion is the whole point. `_fetch_range` cannot tell a complete
    answer from a truncated one, because Savant returns both the same way, so
    the check has to happen here where the range can still be narrowed.
    """
    raw = _fetch_range(start, stop)
    if len(raw) < EXPORT_CAP:
        return raw

    if start >= stop or depth >= MAX_SPLIT_DEPTH:
        log.error("%s..%s returned %d rows, at or above the %d cap, and "
                  "cannot be split further. This day is INCOMPLETE and the "
                  "rates built from it will be wrong.",
                  start, stop, len(raw), EXPORT_CAP)
        return raw

    # Integer days, and floor rather than round. `(stop - start) / 2` yields
    # a timedelta carrying a half-day that date arithmetic silently drops,
    # and a `max(1, ...)` floor is worse still: on a two-day range it puts
    # `mid` ON `stop`, so the right half becomes stop+1..stop - a backwards
    # range that Savant would answer with something, and nobody would know
    # which days it was for. Flooring makes the halves (start..mid) and
    # (mid+1..stop), which are adjacent, non-overlapping, and both non-empty
    # for any range of two days or more.
    span = (stop - start).days
    mid = start + timedelta(days=span // 2)
    log.info("%s..%s hit the %d-row cap - splitting", start, stop, EXPORT_CAP)
    left = _fetch_window(start, mid, depth + 1)
    time.sleep(PAUSE_SECONDS)
    right = _fetch_window(mid + timedelta(days=1), stop, depth + 1)
...
def _fetch_range(start: date, stop: date) -> pd.DataFrame:
    ...
    May be TRUNCATED at EXPORT_CAP rows. Callers must go through
    `_fetch_window`, which detects that and narrows the range.
    """
...
def fetch_one(season: int) -> pd.DataFrame:
    ...
    # Completeness, checked rather than assumed. A regular season is 2,430
    # games; with spring training and the postseason in the window this
    # should comfortably exceed it for a finished season. Falling short means
    # the fetch dropped games, which is precisely what the export cap did
    # silently the first time, so it is stated as a number rather than left
    # to be noticed.
    games = int(df["game_pk"].nunique())
    log.info("season %d: %d player-games (%d batter, %d pitcher) across %d "
             "distinct games", season, len(df),
             int((df["side"] == "batter").sum()),
             int((df["side"] == "pitcher").sum()), games)
    finished = date(season, *SEASON_END) < date.today()
    if finished and games < 2430:
        log.error("season %d is over and only %d games were fetched, against "
                  "a 2,430-game regular season. Pitches are MISSING - check "
                  "for cap warnings above.", season, games)
    per_game = len(df) / games if games else 0
    if games and not (20 <= per_game <= 40):
        log.warning("season %d averages %.1f player-games per game, which is "
                    "outside the 20-40 a nine-inning game produces. The "
                    "reduction may be wrong.", season, per_game)
    return df
